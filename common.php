...
//header('Location:http://wavework2.kr/503.html');
...
//==========================================================================================================================
if ($_SERVER['REMOTE_ADDR'] != "172.30.1.35") {
...
if (isset($_REQUEST['wr_18'])) { // 리스트 페이지
    $wr_18 = $_REQUEST['wr_18'];
    if ($wr_18) {
        $qstr .= '&amp;wr_18=' . urlencode($wr_18);
    }
} else {
    $wr_18 = '';
}
...
define("PAY_TYPE", array(1 => "카드결제",2 => "직발주"));
// define("PLATFORM_TYPE",array("1000"=>"한국","3000"=>"미국","4000"=>"FBA","5000"=>"W-FBA","6000"=>"U-FBA","7000"=>"한국반품","8000"=>"미국반품", "11000" => "한국폐기", "12000" => "미국폐기"));

$warehouse_datafile = $_SERVER['DOCUMENT_ROOT'] . '/warehouse/warehouse_list.php';
require_once $warehouse_datafile;

$platformTypeArray = [];

// $warehouseConfig 배열의 모든 항목을 순회합니다.
foreach ($warehouseConfig as $code => $config_warehouse) {
    $platformTypeArray[$code] = $config_warehouse['ware_name'];
}

// PLATFORM_TYPE 상수를 정의합니다.
// define() 함수는 PHP 7.0부터 배열을 값으로 정의할 수 있습니다.
if (!defined('PLATFORM_TYPE')) { // 상수가 이미 정의되어 있지 않은지 확인하는 것이 좋습니다.
    define("PLATFORM_TYPE", $platformTypeArray);
}

define("CODE_TYPE", ["1" => "브랜드","2" => "카테고리","3" => "배송코드","4" => "매출처","5" => "매입처","6" => "환율","7" => "카드"]);
