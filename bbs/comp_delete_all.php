...
// 매출자료 관리 완전삭제 삭제 12.21
if($_POST['bo_table']=="sales"){
    for($i=0;$i < count($_POST['chk_wr_id']);$i++){
        $order_num = sql_fetch("SELECT wr_subject FROM g5_write_sales WHERE wr_id = '{$_POST['chk_wr_id'][$i]}' ")['wr_subject'];
        
        sql_query("DELETE FROM g5_write_sales WHERE wr_subject = '{$order_num}' ");   //매출자료 관리 
        sql_query("DELETE FROM g5_sales0_list WHERE wr_order_num = '{$order_num}' ");  //매출 관리
        sql_query("DELETE FROM g5_sales1_list WHERE wr_order_num = '{$order_num}' ");  //발주 관리
        sql_query("DELETE FROM g5_sales2_list WHERE wr_order_num = '{$order_num}' ");  //입고 관리
        sql_query("DELETE FROM g5_sales3_list WHERE wr_order_num = '{$order_num}' ");  //출고 관리
