...
for ($i=$chk_count-1; $i>=0; $i--)
{
	...
	# 매출에 등록된 데이터인지 확인
	$sql = "SELECT * FROM g5_sales0_list WHERE wr_id = '".$tmp_array[$i]."'";
	$chk = sql_fetch($sql);
	if($chk['seq']){
		continue;
	}
    ...
    $sql = " select count(*) as cnt from $write_table
                ...
                and wr_is_comment = 0 ";
    $row = sql_fetch($sql);
    if ($row['cnt'] && $write_table != "g5_write_sales")
            continue;
	...
	if($write_table == "g5_write_product"){
		sql_query(" UPDATE {$write_table} SET wr_delYn='Y',wr_last = NOW() WHERE wr_parent = '{$write['wr_id']}' ");
	}else{
