...
if ($w == '' || $w == 'r') {
	...
	// 제품관리의 경우 추가 항목이 있어 해당 항목만 UPDATE
	if($write_table == "g5_write_product"){
		$sql = "UPDATE g5_write_product SET taxType='".$taxType."' WHERE wr_id = '".$wr_id."'";
		sql_query($sql);
	}
	...
	# 엑셀 자료 중 매출등록이 되지 않은 주문건 중 같은 SKU의 데이터 상품번호 업데이트
	if($swr_id != ""){
		$sql = "SELECT A.* FROM g5_write_sales A \n";
		$sql .= "INNER JOIN g5_write_sales B ON B.wr_16 = A.wr_16 AND B.wr_17 = A.wr_17 \n";
		$sql .= "LEFT OUTER JOIN g5_sales0_list C ON C.wr_order_num LIKE CONCAT('%',A.wr_subject) \n";
		$sql .= "WHERE B.wr_id='".$swr_id."' AND C.seq IS NULL";
		$rs = sql_query($sql);
		while($row = sql_fetch_array($rs)){
			$sql = "UPDATE g5_write_sales SET wr_product_id='".$wr_id."' WHERE wr_id='".$row['wr_id']."'";
			sql_query($sql);
		}
	}
	...
	if($write_table == "g5_write_product"){
		...
		$sql .= "WHERE B.seq IS NULL AND wr_16='".$wr_1."'";
		$rs = sql_query($sql);
		while($chk = sql_fetch_array($rs)){
			$sql = "UPDATE g5_write_sales SET wr_product_id='".$wr['wr_id']."' WHERE wr_id='".$chk['wr_id']."'";
			sql_query($sql);
		}
	}
...
# 제품 수정 또는 등록일 경우 ( g5_write_product_fee )
if($write_table == "g5_write_product"){

	$sql = "DELETE FROM g5_write_product_fee WHERE wr_id='".$wr_id."'";
	sql_query($sql);
	...
	if(count($domain) > 0){
		foreach($domain as $key => $val){
			$domain_val = $val;
			$warehouse_val = $warehouse[$key];
			$product_fee_val = $product_fee[$key];
			$fee_type_val = "1";
			$paypal_fee_val = $paypal_fee[$key];
			$grant_price_val = $grant_price[$key];
			$FBA_fee_val = $FBA_fee[$key];
			
			$sql = "INSERT INTO g5_write_product_fee(wr_id, domain, warehouse, fee_type, product_fee,paypal_fee,grant_price,FBA_fee,regdate)";
			$sql .= "VALUES('".$wr_id."','".$domain_val."','".$warehouse_val."', '".$fee_type_val."','".$product_fee_val."','".$paypal_fee_val."','".$grant_price_val."','".$FBA_fee_val."',NOW())";
			sql_query($sql);
