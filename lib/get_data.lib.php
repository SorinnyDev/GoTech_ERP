...
function get_menu_db($use_mobile=0, $is_cache=false, $usa=""){
    global $g5;
    ...
    if( !($cache[$key] = run_replace('get_menu_db', array(), $use_mobile)) ){
        ...
        for ($i=0; $row=sql_fetch_array($result); $i++) {
            ...
            $row['me_menu_num'] = $row['me_menu_num'];
            ...
            for ($k=0; $row2=sql_fetch_array($result2); $k++) {
				$row2['me_id'] = $row2['me_id'];
				$row2['me_menu_num'] = $row2['me_menu_num'];
