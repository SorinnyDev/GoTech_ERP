...
function download_file_nonce_is_valid($nonce, $bo_table, $wr_id)
{
    ...
    if (sha1($salt . $secret . $maxTime) !== $hash) {
        return false;
    }
    if (G5_SERVER_TIME > (int)$maxTime) {
        return false;
    }
...
function sql_connect($host, $user, $pass, $db = G5_MYSQL_DB)
{
    ...
    if (function_exists('mysqli_connect') && G5_MYSQLI_USE) {
        ...
        $link = @mysqli_connect($host, $user, $pass, $db) or die('Mysql Error');
...
# 연관배열로 반환 함수
function sql_fetch_all($sql, $link = null)
{
    $result = sql_query($sql, $link);
    $rows = [];
...
# 트랜잭션 시작
function sql_trans_start()
{
    sql_query("START TRANSACTION");
}

# 트랜잭션 롤백
function sql_trans_rollback()
{
    sql_query("ROLLBVACK");
}

# 트랜잭션 커밋
function sql_trans_commit()
{
    sql_query("COMMIT");
}
...
function is_nan_check($NaN_check)
{
    if (is_nan($NaN_check)) {
        $NaN_check = 0;
    }
...
{
    //if($_SERVER['REMOTE_ADDR']=='172.30.1.21' || $_SERVER['REMOTE_ADDR']=='172.30.1.35'){
    if ($_SERVER['REMOTE_ADDR'] == '211.210.172.98') {
        ############################################
        #json 데이터인지 확인
        ############################################
...
if (!function_exists("get_country")) {
    function get_country()
    {
        ...
        $sql = "SELECT * FROM g5_country_code ORDER BY code_en ASC";
        $rs = sql_query($sql);
...
# 환율 정보 가져오기
if (!function_exists("fnGetExchage")) {
    function fnGetExcharge($ex_eng, $getDt = "")
    {
        if ($getDt) {

            $getDt = date("Y-m-d", strtotime($getDt));
            if (date("Y-m-d") != $getDt) {
                //$table = "g5_excharge_".date("Ym",strtotime($getDt));
                $table = "g5_excharge_log";
                $sql = "SELECT * FROM " . $table . " WHERE ex_eng='" . $ex_eng . "' AND ex_date='" . $getDt . "'";
            } else {
                $table = "g5_excharge";
                $sql = "SELECT * FROM " . $table . " WHERE ex_eng='" . $ex_eng . "'";
            }
        } else {
            $table = "g5_excharge";
            $sql = "SELECT * FROM " . $table . " WHERE ex_eng='" . $ex_eng . "'";
...
if (!function_exists('isDefined')) {
    function isDefined($str)
    {
        ...
        if (is_array($str)) {
            ...
            if ($str_temp != "undefined" and $str_temp != "" and $str_temp != "null" and $str_temp != "NaN") {
                $isResult = true;
            } else {
...
if (!function_exists('date_diff2')) {
    ...
    function date_diff2($date1, $date2)
    {
        ...
        if ($tmp1 > $tmp2) {
            $diff = ($tmp1 - $tmp2) / 86400;
        } else {
            $diff = ($tmp2 - $tmp1) / 86400;
        }
...
if (!function_exists('date_to_date')) {
...
// 게시판의 다음글 번호를 얻는다.
function get_acc5_set_count($wr_ori_order_num)
{

    $row = sql_fetch(" SELECT count(seq) as seq FROM g5_sales3_list WHERE wr_ori_order_num='$wr_ori_order_num' ");

    return $row['cnt'];
}


function get_pm_price($wr_ori_order_num_val, $exchange_rate, $wr_domain, $stwr_domaindate, $ed_date)
{
	...
	WHERE A.wr_release_use='1' AND A.wr_domain = '" . $wr_domain . "' AND A.wr_date4 BETWEEN '" . $st_date . "' AND '" . $ed_date . "' 
...
//
// 업로드함수 FileUploadName() 수정 : UTF-8로 변환으로 인해 수정 (20090820 )
// 업로드함수 FileUploadName() 수정 : UTF-8로 변환으로 인해 한글파일명 문제 생기는걸 방지하기 위해
// 임의의 파일명 생성하는것으로 변경 (이게 제일 나은 방법인거 같음) (20090917 )
// 임의의 파일명 생성 함수 : getUniqueFileName() (20090917 )
// 실제 파일명을 저장하는 작업은 나중에 하도록 할것임 (20091217 )
...
function isDebug()
{
    ...
    $debug_ip_prefix = '58.122.170.'; // IP 대역

    if ($member['mb_id'] == 'test' || strpos($_SERVER['REMOTE_ADDR'], $debug_ip_prefix) === 0) {
        return true;
    }
...
/**
 * JSON 형식으로 응답을 반환하는 함수
 *
 * @param bool $success 성공 여부
 * @param string $message 응답 메시지
 * @param mixed $data 반환할 데이터 (선택 사항)
 * @param string $redirect 리다이렉트 URL (선택 사항)
 * @param int $status_code HTTP 상태 코드 (기본값: 200)
 * @param bool $exit_after 함수 종료 후 스크립트 실행 중단 여부 (기본값: true)
 * @return string JSON 응답
 */
function json_response($success = true, $message = '', $data = null, $redirect = '', $status_code = 200, $exit_after = true)
{
    // 헤더 설정
    ...
    if ($json_result === false) {
        ...
        $json_result = json_encode($error_response, JSON_UNESCAPED_UNICODE);
    }
 ...
 * @return bool 비어있지 않으면 true, 비어있으면 false 반환
 */
function isNotEmpty($var)
{
    // null, 빈 문자열, 0, '0', [], false 등을 모두 비어있는 것으로 간주
 ...
 *
 * @param mixed $var 확인할 변수
 * @return bool 비어있으면 true, 비어있지 않으면 false 반환
 */
 ...
 * 쿼리 액션 로그 저장
 */
function log_query_action($sql, $link)
{
    global $member;

    $sql_upper = strtoupper(trim($sql));

    // INSERT, UPDATE, DELETE 쿼리만 로그 저장
    if (!preg_match('/^(INSERT|UPDATE|DELETE)\s+/i', $sql_upper)) {
        return;
    }

    // g5_login 관련된거 저장 안함
    if (strpos($sql, 'g5_login') !== false) {
        return;
    }
    ...
    // 테이블명 추출
    $table_name = extract_table_name($sql, $query_type);

    // 로그 테이블 자체는 로깅하지 않음
    if ($table_name === 'g5_query_logs') {
        return;
    }

    // 사용자 정보
    $mb_id = isset($member['mb_no']) ? (int)$member['mb_no'] : 0;
    ...
    // 로그 저장 쿼리 생성
    $log_sql = "INSERT INTO g5_query_logs 
                (mb_id, query_type, table_name, sql_query, ip_address, page_url) 
                VALUES (
                    " . $mb_id . ",
                    '" . mysqli_real_escape_string($link, $query_type) . "',
                    '" . mysqli_real_escape_string($link, $table_name) . "',
                    '" . mysqli_real_escape_string($link, $sql) . "',
                    '" . mysqli_real_escape_string($link, $ip_address) . "',
                    '" . mysqli_real_escape_string($link, $page_url) . "'
                )";

    // 로그 저장 (에러가 발생해도 원본 쿼리에 영향 없도록)
    if (function_exists('mysqli_query') && G5_MYSQLI_USE) {
 ...
 * 쿼리에서 테이블명 추출
 */
function extract_table_name($sql, $query_type)
{
    $table_name = '';
    ...
    switch ($query_type) {
        ...
        case 'UPDATE':
            if (preg_match('/UPDATE\s+([^\s]+)\s+SET/i', $sql, $matches)) {
                $table_name = trim($matches[1], '`');
            }
            break;
