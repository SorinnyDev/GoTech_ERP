...
?>
      ...
      <form name="fboardlist" id="fboardlist" action="<?php echo G5_BBS_URL; ?>/board_list_update.php" onsubmit="return fboardlist_submit(this);" method="post">
        ...
        <div class="tbl_head01 tbl_wrap" style="overflow-x:scroll;height:600px;">
          <table style="width:3500px;position:sticky;top:0;background:#fff;">
            ...
            <tr id="tr-dodoskin" style="display: none;">
              <th class="sticky-th" style="width:100px;">구분</th>
              <th class="sticky-th" style="width:80px;">날짜</th>
              <th class="sticky-th" style="width:100px;">주문번호</th>
              <th class="sticky-th" style="width:300px;">상품명</th>
              <th class="sticky-th" style="width:200px;">SKU</th>
              ...
              <th class="sticky-th" style="width:100px;">신고가격</th>
              <th class="sticky-th" style="width:100px;">매출 * 환율(원)</th>
              <th class="sticky-th" style="width:100px;">TAX-U</th>
              <th class="sticky-th" style="width:100px;">TAX</th>
              <th class="sticky-th" style="width:100px;">Shipping</th>
              <th class="sticky-th" style="width:100px;">손익(원)</th>
              <th class="sticky-th" style="width:100px;">이익률(%)</th>
              <th class="sticky-th" style="width:100px;">부가세환급(원)</th>
            </tr>
  ...
  <div class="bo_sch_wrap">
    <fieldset class="bo_sch">
      ...
      <form name="fsearch" method="get" onsubmit="return false;">
        ...
        <!-- 주문번호 검색 -->
        <div style="border:1px solid #ddd; margin-bottom:20px; padding:10px">
          <input type="text" name="ordernum" value="" class="frm_input" style="width:100%;" placeholder="주문번호 조회">
        </div>
  ...
  <script>
    ...
    // 손익보고서 리스트 불러오기
    async function fnLoadList() {
      const params = $("form[name='fsearch']").serialize();
      const ordernum = document.querySelector('[name=ordernum]').value;

      let domain = $("[name=wr_18]").val();

      if (ordernum.length > 0) {
        try {
          domain = await $.post('./ajax.acc5_fetch_search_type.php', {ordernum});
        } catch (error) {
      ...
      let requestURL;
      if (domain === 'dodoskin') {
        requestURL = './ajax.acc5_list_dodoskin.php';
        $("#tr-dodoskin").show();
        $("#tr-default").hide();
      } else {
        requestURL = './ajax.acc5_list.php';
        $("#tr-dodoskin").hide();
        $("#tr-default").show();
      }
    ...
    function fnExcelDown() {
      ...
      let url;
      if ($("[name=wr_18]").val() === 'dodoskin') {
        url = './acc5_list_dodoskin_excel.php';
      } else {
        url = './acc5_list_excel.php';
      }
