...
if ($sc_orderer == "" || $type == "orderer") {
  $sql = "
    select cl.code_name as orderer_name,
           ...
           (select sum(amount) as amount from g5_acc2_carryover_amount as ca where ca.wr_orderer = sl.wr_orderer and base_date <= '2025-02-07') as carryover
    ...
    WHERE sl.wr_direct_use = '0' and sl.wr_date3 BETWEEN '$st_date' AND '$ed_date' {$sql_where}
    group by sl.wr_orderer
    order by cl.code_name, wr_date3 desc
	";
  ...
  while ($row = sql_fetch_array($rs)) {
    ...
    $list[$orderer_name]['data'][$wr_date3] = $row;
  }
}
  ...
  <div id="bo_list">
    <div class="bo_list_innr">
      ...
      <form name="fboardlist" id="fboardlist" action="<?php echo G5_URL; ?>/acc/acc3_list_update.php" onsubmit="return acc3_frm_submit(this);" method="post">
        ...
        <p>
          2025/1/1 부터 <strong>기준일자</strong>까지의 합산 금액을 보여줍니다.<br>
        </p>
        <div id="bo_li_top_op">
