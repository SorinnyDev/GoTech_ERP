      ...
      <form name="acc2_frm" id="acc2_frm" action="<?php echo G5_URL; ?>/acc/acc2_list_update.php" method="post">
        ...
        <div id="bo_li_top_op">
          ...
          <ul class="<?php echo isset($view) ? 'view_is_list btn_top' : 'btn_top2'; ?>">
            ...
            <li>
              <button type="button" value="선택입금" onclick="fnPayOrder();" class="btn_b02 "
                      style="background:#8e74ef;color:white;"><i class="fa fa-file-pdf-o"></i> 선택지급
              </button>
            </li>
            <?php if ($code) { ?>
              <li>
        ...
        <div class="tbl_head01 tbl_wrap" style="overflow-x: auto;">
          <table style="width: 100%; min-width: 1200px;">
            ...
            <?php if ($code) { ?>
              <tr>
                <td style="text-align: center; background:rgb(232, 239, 243);" colspan="10"><strong>이월 미지급 합산 금액</strong></td>
                <td colspan="1" style="background:rgb(232, 239, 243);">₩<?= number_format($sum_misu + $carryover) ?></td>
                <td colspan="2" style="text-align: center; background:rgb(232, 239, 243);"></td>
              </tr>
              <tr>
                <td style="text-align: center; background:rgb(232, 239, 243);" colspan="10"><strong>이월 보정 금액</strong></td>
              ...
              <?php foreach ($list as $k => $v) {
                foreach ($v as $k2 => $v2) {
                  ...
                  foreach ($v2 as $k3 => $item) {
                    ...
                    <tr class="<?= $tr ?>">
                      <td align="center"><input type="checkbox" name="seq_arr[]" value="<?= $item['seq'] ?>"/>
                      </td>
                      <td><?= $item['wr_date3'] ?></td>
                      ...
                      <td align="center">
                        <input type="text" name="total_price[<?= $item['seq'] ?>]" value="<?= $item['wr_order_total'] ?>"
                      ...
                      <td align="center">
                        ...
                        <input type="text" name="pay_price[<?= $item['seq'] ?>]" data="<?= $item['seq'] ?>"
                               class="frm_input pay_price" value="<?= (int)$item['pay_price'] ?>"/>
                      </td>
                      <td align="center">
                        <input type="text" name="misu[<?= $item['seq'] ?>]" data="<?= $item['seq'] ?>"
                               class="frm_input misu readonly" value="<?= $item['wr_misu'] ?>" readonly/>
                      </td>
                      <td align="center">
                      ...
                      <td align="center">
                        <a href="javascript:void(0);" class="btn_b01" onclick="fnPayUnit('<?= $item['seq'] ?>');">지급</a>
                      </td>
                    </tr>
  ...
  <div class="bo_sch_wrap edit-modal">
    <fieldset class="bo_sch" style="padding:10px">
      ...
      <form name="famount" method="get">
        ...
        <input type="hidden" name="base_date" value="<?=$st_date?>">
        <div style="margin-bottom: 15px;">
          <div class="sch_bar" style="margin-top:3px; display: flex; justify-content: center;">
            <input type="date" name="date" value="<?php echo $st_date ?>" required class="sch_input" size="25"
                   maxlength="20" placeholder="" style="width:45%;text-align:center" disabled>
          </div>
        </div>
        <div style="margin-bottom: 15px;">
        ...
        <div style="margin-bottom:15px;">
          <input type="number" name="amount" value="<?=$carryover?>" class="frm_input" style="width:100%;"
  ...
  <script>
    ...
    $(function () {
      ...
      $("#payAll").click(function () {
        ...
        $(".pay_price").each(function () {
          ...
          if (flag == true) {// 일괄지급 체크시
            var total_price = $("input[name='total_price[" + seq + "]']").val();
            $("input[name='pay_price[" + seq + "]']").val(total_price);
            $("input[name='misu[" + seq + "]']").val(0);

          } else { // 일괄지급 미체크시
            var total_price = $("input[name='total_price[" + seq + "]']").val();
            var ori_pay_price = $("input[name='ori_pay_price[" + seq + "]']").val();
      ...
      $(".wr_warehouse_price").bind("keyup", function () {
        ...
        var total_order_price = $("input[name='wr_total_price[" + seq_arr + "]']").val();
    ...
    function fnPayUnit(seq) {
      ...
      var param = {"mode": "<?= $column ?>", "seq": seq, "pay_price": pay_price, "misu": misu};
      $.ajax({
    ...
    function select_pay(seq, type) {
      ...
      $.post(g5_bbs_url + "/ajax.misu_update.php", {wr_misu: wr_misu, type: type, seq: seq}, function (data) {
        if (data == 'y') {
          alert("완료되었습니다.");
          location.reload();
        } else if (data == 'n') {
