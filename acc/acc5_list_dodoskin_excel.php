...
header("Content-Disposition: attachment; filename=손익자료_도도스킨_" . date("Ymd_His") . ".xls");
...
SELECT  
    ...
    A.wr_order_num, IFNULL(B.ibgo_danga, C.wr_22) AS danga,
...
";
  ...
  foreach ($sets as $ori_id => $set) {
    ...
    foreach ($set['rows'] as $row) {
      $is_set = !empty($row['wr_set_sku']);
      $group = $is_set ? '세트상품' : (substr_count($wr_ori_order_num_list, $ori_id) > 1 ? '합배송' : '단일배송');
    ...
    "<td></td>",
      ...
      "<td>" . number_format($summary['wr_shipping_price']) . "</td>",
