...
$msg = ($_POST['btn_submit']=="선택삭제") ? "[매출등록] 데이터가 삭제되었습니다." : "[매출등록] 데이터가 완전히 삭제되었습니다.";
$cancel_arr = array();
$total = $count;
$succ = 0;
$fail = 0;
for($i=0; $i<$count; $i++) {
	$obj = sql_fetch("select * from g5_sales0_list where seq = '{$_POST['seq'][$i]}' ");
	if($_POST['btn_submit']=="선택삭제"){
		...
		# 발주 정보 삭제
		$sql = "DELETE FROM g5_sales1_list WHERE wr_order_num = '".$obj['wr_order_num']."'";
		sql_query($sql);

		$sql = "INSERT INTO g5_del_history(query_string,params,mb_id,reg_date,wr_order_num)VALUES('".addslashes($sql)."','".addslashes(json_encode($_REQUEST))."','".$member['mb_id']."',NOW(),'".$obj['wr_order_num']."')";
		sql_query($sql);
		
		# 하위 단계(발주,입고, 출고) 데이터 처리 후 마지막으로 매출 데이터 삭제
        $sql = "delete from g5_sales0_list where seq = '{$_POST['seq'][$i]}' LIMIT 1";
        sql_query($sql);
        ...
        sql_query("delete from g5_write_sales where wr_subject = '{$wr_order_num}' ");
        sql_query("delete from g5_sales0_list where wr_order_num = '{$wr_order_num}' ");
        sql_query("delete from g5_sales1_list where wr_order_num = '{$wr_order_num}' ");
        sql_query("delete from g5_sales2_list where wr_order_num = '{$wr_order_num}' ");
        sql_query("delete from g5_sales3_list where wr_order_num = '{$wr_order_num}' ");
