...
.y_ea1 { font-weight:600; color:blue !important}
...
<div id="bo_list">
	<div class="bo_list_innr">
		<h2 class="board_tit">미국 출고대기</h2>
		<form name="fboardlist" id="fboardlist" action="./us_wait_list_update.php" onsubmit="return fboardlist_submit(this);" method="post">
	    ...
	    <div id="bo_li_top_op">
			...
			<ul class="<?php echo isset($view) ? 'view_is_list btn_top' : 'btn_top2';?>">
			  ...
			  <li><button type="submit" name="btn_submit" class="btn_b01" value="일괄출고" onclick="document.pressed=this.value"></i> 선택 일괄출고</button></li>
	    ...
	    <div id="bo_li_01" style="overflow-x:scroll;height:400px">
	        ...
	        <div id="bo_li_01" class="list_03">
		        <ul style="width:3500px;">
					...
					$sql_search .= " and a.wr_warehouse = '3000' and a.wr_chk = 0";
					...
					where (1) {$sql_search} group by a.seq {$sql_order} ";
					...
					for ($i=0; $row=sql_fetch_array($rst); $i++) {
						...
						//$move = sql_fetch("select * from g5_stock_move where product_id = '{$row['wr_product_id']}' and wr_state = 0 and wr_type = 1");
						$move = sql_fetch("select SUM(wr_stock) AS wr_stock from g5_stock_move where product_id = '{$row['wr_product_id']}' and wr_state = 0 AND wr_in_warehouse='3000'");
						$move_stock = (int)$move['wr_stock'];
						...
						$ea_chk = $row['wr_36'] < $row['wr_ea'] ? 'no_ea1' : 'y_ea1';
		            ...
		            <li class="modify" data="<?php echo $row['seq']?>">
		                ...
		                <div class="cnt_left <?php echo $ea_chk?>" style="width:100px;text-align:right"><?php echo $row['wr_36'] ?></div>
						 <div class="cnt_left" style="width:100px;text-align:right"><?php echo $move_stock ?></div>
		...
		<div style="border:1px solid #ddd; width:100%; height:580px" id="result_form">
...
function fboardlist_submit(f) {
    ...
    if(document.pressed == "일괄출고") {

        if (!confirm("선택한 데이터를 정말 일괄출고 하시겠습니까?"))
            return false;

        f.removeAttribute("target");
        f.action = "./us_wait_list_update.php";
    }
