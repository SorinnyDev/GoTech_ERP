...
for ($i=0; $i<$count_chk_wr_id; $i++) {
    ...
    $expired_date_val = $_POST['expired_date'][$wr_id_val] ?? null;

    // 유통기한이 입력되어 있는 경우에만 유효성 검증
    if (!empty($expired_date_val)) {
        // YYYY-MM-DD 형식 검증
        if (!preg_match('/^\d{4}-\d{2}-\d{2}$/', $expired_date_val)) {
            alert('유통기간이 유효하지 않습니다.');
        }

        // 날짜 유효성 검증
        $date_parts = explode('-', $expired_date_val);
    ...
    (!empty($expired_date_val) ? ", expired_date = '{$expired_date_val}'" : "")."
    ";
    sql_query($sql, true);
    ...
    // 유통기한 저장 - 유통기한이 입력되어 있는 경우에만
    if (!empty($expired_date_val)) {
        if($excel['wr_etc_chk'] != 1) {
            // 일반 주문건 - 이미 정의된 $rack 사용
            $sql4 = "INSERT INTO g5_rack_expired SET rack_id = '{$rack['seq']}', product_id = '{$item['wr_id']}', expired_date = '{$expired_date_val}', sales2_seq = '{$sales2_id}'";
        } else {
            // 기타 주문건 - 임시창고 랙 사용
            $temp_rack = sql_fetch("select seq from g5_rack where gc_warehouse = '9000'");
            if($temp_rack['seq']) {
                $sql4 = "INSERT INTO g5_rack_expired SET rack_id = '{$temp_rack['seq']}', product_id = '{$item['wr_id']}', expired_date = '{$expired_date_val}', sales2_seq = '{$sales2_id}'";
            }
        }
