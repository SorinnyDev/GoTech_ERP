...
if ($warehouse == 1000)
	...
	$sheet->setCellValue('A1', '출고등록(미국)');
else if ($warehouse == 7000)
	$sheet->setCellValue('A1', '출고등록(한국반품)');
else if ($warehouse == 8000)
	$sheet->setCellValue('A1', '출고등록(미국반품)');
// $sheet->setCellValue('A1', '출고등록(한국)');
...
while ($row = sql_fetch_array($rst)) {
  ...
  $sheet->setCellValueExplicit('D' . $rowNum, $row['wr_order_num'], PHPExcel_Cell_DataType::TYPE_STRING);
	...
	$sheet->setCellValue('F' . $rowNum, $row['wr_deli_nm']);
	...
	$sheet->setCellValue('I' . $rowNum, $row['wr_deli_addr1']);
	$sheet->setCellValue('J' . $rowNum, $row['wr_deli_addr2']);
	$sheet->setCellValue('K' . $rowNum, $row['wr_deli_city']);
	$sheet->setCellValue('L' . $rowNum, $row['wr_deli_ju']);
	$sheet->setCellValue('M' . $rowNum, $row['wr_deli_zip']);
	$sheet->setCellValue('N' . $rowNum, $row['wr_deli_country']);
	$sheet->setCellValue('O' . $rowNum, $rack_name);
	$sheet->setCellValue('P' . $rowNum, $row['wr_date']);
	$sheet->setCellValue('Q' . $rowNum, $row['wr_date2']);
	$sheet->setCellValue('R' . $rowNum, $row['wr_date3']);
	$sheet->setCellValue('S' . $rowNum, $row['wr_date4']);
	$sheet->setCellValueExplicit('T' . $rowNum, $product['wr_1'], PHPExcel_Cell_DataType::TYPE_STRING);
	$sheet->setCellValue('U' . $rowNum, $product['wr_2']);
	$sheet->setCellValue('V' . $rowNum, $row['wr_product_nm']);
	$sheet->setCellValue('W' . $rowNum, $product['wr_subject']);
	$sheet->setCellValueExplicit('X' . $rowNum, $product['wr_5'], PHPExcel_Cell_DataType::TYPE_STRING);
	$sheet->setCellValueExplicit('Y' . $rowNum, $row['wr_hscode'], PHPExcel_Cell_DataType::TYPE_STRING);
	$sheet->setCellValue('Z' . $rowNum, $row['wr_ea']);
	$sheet->setCellValue('AA' . $rowNum, 1); #박스수 일괄 1 고정
	$sheet->setCellValue('AB' . $rowNum, $row['wr_danga']);
	$sheet->setCellValue('AC' . $rowNum, $row['wr_singo']);
	$sheet->setCellValue('AD' . $rowNum, $row['wr_currency']);
	$sheet->setCellValue('AE' . $rowNum, $row['wr_weight1']);
	$sheet->setCellValue('AF' . $rowNum, $row['wr_weight2']);
	$sheet->setCellValue('AG' . $rowNum, $row['wr_servicetype']);
	$sheet->setCellValue('AH' . $rowNum, $row['wr_deli_country']);
	$sheet->setCellValue('AI' . $rowNum, $delivery_nm);
	$sheet->setCellValue('AJ' . $rowNum, $delivery_fee);
	$sheet->setCellValue('AK' . $rowNum, $delivery_fee2);
	$sheet->setCellValue('AL' . $rowNum, $delivery_oil);
	$sheet->setCellValue('AM' . $rowNum, $delivery_fee + $delivery_fee2 + $delivery_oil);
	$sheet->setCellValue('AN' . $rowNum, $row['wr_release_etc']);
