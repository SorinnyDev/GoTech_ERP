		...
		<form name="fboardlist" id="fboardlist" action="./sales3_list_update.php" onsubmit="return fboardlist_submit(this);"
			...
			<div id="bo_li_top_op">
				<div class="bo_list_total">
					<? if (in_array($warehouse, ['4000', '5000', '6000'])) { ?>
						<span>
          ...
          <li class="wli_cnt">
            <select class="frm_input" name="nm_type" id="nm_type" style="height:37px">
          ...
          <li class="wli_cnt">
            ...
            if ($serviceType === "0001") {
              ...
              $sql_search .= " and A.wr_servicetype != '0001'";
						...
						if ($directUse === "1") {
              $sql_search .= " and A.wr_direct_use = 1";
            } else if ($directUse === "2") {
              $sql_search .= " and A.wr_direct_use = 0";
            }

            if (!$sst && !$sod) {
              $sql_order = "order by wr_ori_order_num asc, wr_order_num asc ";
						} else {
              $sql_order = "order by $sst $sod";
            }

						$sql = "select A.*, B.hap_cnt, C.sales3_cnt, wp.wr_subject, (CASE WHEN IFNULL(wr_set_sku,'') > '' THEN CONCAT(A.wr_ori_order_num,'_',A.wr_set_sku) ELSE '' END) AS set_order from g5_sales3_list A 
								LEFT OUTER JOIN(
						...
						for ($i = 0; $row = sql_fetch_array($rst); $i++) {
							...
							<li class="modify tr_<?= $only_number_odnum ?> <?= $li_bg ?>" data="<?php echo $row['seq'] ?>">
								...
								<div class="cnt_left" style="width:250px;<?php echo $imsi_item ?>"><?= $nm_type === 'type_1' ? $row['wr_subject'] : $row['wr_product_nm'] ?>
								</div>
	...
	<fieldset class="bo_sch" style="top: 35%;">
      ...
      <div style="border:1px solid #ddd; margin-bottom:10px; padding:10px">
        ...
        <label><input type="radio" name="serviceType" value="1000" <?php echo get_checked($serviceType, '1000') ?>> 특송 외 출력</label>
			...
			<div style="border:1px solid #ddd; margin-bottom:10px; padding:10px">
        <label><input type="radio" name="directUse" value="0" <?php echo get_checked($directUse, 0) ?>> 전체</label>
        <label><input type="radio" name="directUse" value="1" <?php echo get_checked($directUse, 1) ?>> 바로출고만</label>
        <label><input type="radio" name="directUse" value="2" <?php echo get_checked($directUse, 2) ?>> 바로출고 제외</label>
      </div>
			<div style="border:1px solid #ddd; margin-bottom:10px; padding:10px">
...
<script>
  const mb_id = '<?= $member['mb_id'] ?>';
    ...
    if ((mb_id == '9000' || mb_id == 'test') && confirm('최저 배송비 로직을 적용 하시겠습니까?')) {
      excelForm.append($("<input/>", { type: "hidden", name: "is_delivery_calc", value: 'Y' }));
    }
	...
	$(function () {
      ...
      const original_fee = $('#wr_delivery_fee_original').val();
      const original_fee2 = $('#wr_delivery_fee2_original').val();

      // 배송비 있을 경우 배송비 교체 X
      if (original_fee > 0 || original_fee2 > 0) {
        return;
      }

			const fee = $('#wr_delivery option:selected').attr('data');
      const currency = $('#wr_delivery option:selected').attr('currency');
      let oil_percent = $('#wr_delivery option:selected').attr('oil_percent');
  ...
  window.addEventListener('message', function (event) {
    if (event.data?.event === 'popup_submit') {
      $.post('./sales3_addbox.php', { seq: event.data?.seq, first: 1 }, function (data) {
        $('#result_form').html(data);
      });
    }
  });
