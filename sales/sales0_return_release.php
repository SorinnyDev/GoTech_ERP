...
<div class="new_win">
  <h1><?= $warehouseConfig[$warehouse]['ware_name'] ?> 반품 출고</h1>
    ...
    <div class="tbl_head03 tbl_wrap" style="min-width:1700px;margin-bottom:10px;background: transparent;">
      <table id="releaseTable" style="table-layout: fixed;">
        ...
        <tbody>
          <?php
          $wr_id_arr = array();
          foreach ($_POST['chk_wr_id'] as $val) {
            $wr_id_val = isset($val) ? preg_replace('/[^0-9]/', '', $val) : 0;
            if ($wr_id_val) $wr_id_arr[] = "'" . $wr_id_val . "'";
          }

          $wr_id_arr_str = implode(',', $wr_id_arr);
  ...
  $(document).ready(function() {
    ...
    $('#return_list').on('change', 'input.td_input[name="quantity"]', function() {
      ...
      function handleQuantityUpdate(orderNum, seq, rackName, wr_order, newQty) {
        ...
        if (newQty === 0) {
          if (targetOrder.orderData[seq]['rack_name'] !== undefined) {
            delete targetOrder.orderData[seq]['rack_name'];
            delete targetOrder.orderData[seq]['quantity'];
          ...
          targetOrder.orderData[seq]['rack_name'] = rackName;
          targetOrder.orderData[seq]['quantity'] = newQty;
      ...
      if (totalQuantity === orderQuantity) {
        $(`tr[data-id="${currentOrderNum}"].selected-row`).css('border', '3px solid mediumspringgreen');
        ...
        $(`tr[data-id="${currentOrderNum}"].selected-row`).css('border', '');
