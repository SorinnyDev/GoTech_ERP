	...
	<form name="frm" action="./sales0_search_update.php" method="post" onsubmit="return chkfrm(this)">
						...
						if ($sql_search) {
							$sql = "SELECT
											...
											WHERE
												...
												AND NOT EXISTS (
														SELECT 1
														FROM g5_sales0_list asl
														WHERE asl.wr_id = a.wr_id
												)
											ORDER BY
												a.wr_subject ASC";
		...
		<div class="win_btn btn_confirm" style="position:fixed;bottom:0;width:100%;background:#fff;border-top:1px solid #ddd;margin-bottom:0">
			...
			<select name="wr_warehouse" id="wr_warehouse" required>
				<option value="">선택하세요</option>
				<?php
				foreach ($goodQualityWarehouses as $wareCode => $details) {
				?>
					<option value="<?= $wareCode ?>" <?= get_selected($_GET['warehouse'], $wareCode) ?>><?= $details['ware_name'] ?></option>
				<?php
				} ?>
				<option disabled>───────</option>
				<?php
				foreach ($refurbishedQualityWarehouses as $wareCode => $details) {
				?>
					<option value="<?= $wareCode ?>" <?= get_selected($_GET['warehouse'], $wareCode) ?>><?= $details['ware_name'] ?></option>
				<?php
				} ?>
			...
			<button type="submit" class="btn_submit btn tooltip" style="display:none" id='release_product'><span id="button_label">출고 및 발주생성</span>
				<span class="tooltiptext" id='release_tooltip'>재고가 있는 건은 [출고등록], 재고가없는 건[발주등록]으로 바로등록 됩니다.</span></button>
...
<script>
	function chkfrm(form) {
		const selectedWarehouseCode = $('#wr_warehouse').val();

		console.log('Selected Warehouse Code:', selectedWarehouseCode);

		const json_goodQualityWarehouses = <?= json_encode($goodQualityWarehouses) ?>;
		const json_refurbishedQualityWarehouses = <?= json_encode($refurbishedQualityWarehouses) ?>;

		if (selectedWarehouseCode in json_goodQualityWarehouses) {
			$('.btn_submit').html('<img style="width:15px" src="/mobile/shop/img/loading.gif">');
			$('.btn_submit').attr('disabled', true);
			form.action = './sales0_search_update.php';
			return true;
		} else if (selectedWarehouseCode in json_refurbishedQualityWarehouses) {
			$('.btn_submit').html('<img style="width:15px" src="/mobile/shop/img/loading.gif">');
			$('.btn_submit').attr('disabled', true);
			form.action = './sales0_return_release.php';
			return true;
		} else {
			alert('창고를 선택해주세요.');
			return false;
	...
	document.getElementById('wr_warehouse').addEventListener('change', function() {
		const selectedWarehouseCode = this.value;

		const json_goodQualityWarehouses = <?= json_encode($goodQualityWarehouses) ?>;
		const json_refurbishedQualityWarehouses = <?= json_encode($refurbishedQualityWarehouses) ?>;

		if (selectedWarehouseCode in json_goodQualityWarehouses) {
			$('#button_label').text('출고 및 발주생성');
			$('#release_tooltip').text('재고가 있는 건은 [출고등록], 재고가없는 건[발주등록]으로 바로등록 됩니다.');
		} else if (selectedWarehouseCode in json_refurbishedQualityWarehouses) {
			$('#button_label').text('반품 출고');
			$('#release_tooltip').text('반품이 존재할 때만 출고 등록됩니다.');
		}
	});
</script>
<?php
