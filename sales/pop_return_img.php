...
$row = sql_fetch("select a.*, a.wr_datetime as add_date, b.wr_order_num from g5_return_img a LEFT JOIN g5_return_list b ON(a.return_id=b.seq) where a.seq = '{$seq}'");
if(!$row)
...
<div class="new_win">
    <h1>반품상품 사진확인 - 주문번호 : <?php echo $row['wr_order_num']?></h1>
	...
	<input type="hidden" name="product_id" value="<?php echo $row['return_id']?>">
	...
	<div id="excelfile_upload" class="result_list" style="padding:12px;">
		<?php for($i=1; $i<=5; $i++) {
			...
			if($row['wr_img'.$i]) {
				echo '<img src="'.G5_DATA_URL.'/return/'.$row['wr_img'.$i].'" width="100%" style="max-width:100%"><br>'.PHP_EOL;
			}
		}?>
