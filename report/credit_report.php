...
$cnt = sql_fetch("SELECT SUM(wr_32) as kor, SUM(wr_36) as usa, SUM(wr_37) as tmp, SUM(wr_42) as fba , SUM(wr_43) as wfba , SUM(wr_44) as ufba  {$sql_common} WHERE (1) {$sql_add} ");
?>
...
<div id="bo_list">
	<div class="bo_list_innr">
	    ...
	    <div id="bo_li_top_op">
		    <div class="bo_list_total">
				<div class="local_ov01 local_ov">
                    <span class="btn_ov01" onclick="location.href='<?=G5_URL?>/report/report_list.php?report_type=&report_category=<?=$report_category?>&report_mb_id=<?=$report_mb_id?>&stx2=<?=$stx2?>'">
                    ...
                    <span class="btn_ov01" onclick="location.href='<?=G5_URL?>/report/report_list.php?report_type=wr_32&report_category=<?=$report_category?>&report_mb_id=<?=$report_mb_id?>&stx2=<?=$stx2?>'">
                        <span class="ov_txt <?=$report_type=="wr_32" ? "select_btn" : "" ?>">한국창고</span>
                        <span class="ov_num"><?php echo number_format($cnt['kor']) ?>개</span>
                    </span>
                    <span class="btn_ov01" onclick="location.href='<?=G5_URL?>/report/report_list.php?report_type=wr_36&report_category=<?=$report_category?>&report_mb_id=<?=$report_mb_id?>&stx2=<?=$stx2?>'">
                        <span class="ov_txt <?=$report_type=="wr_36" ? "select_btn" : "" ?>">미국창고</span>
                        <span class="ov_num"><?php echo number_format($cnt['usa']) ?>개</span>
                    </span>
                    <span class="btn_ov01" onclick="location.href='<?=G5_URL?>/report/report_list.php?report_type=wr_42&report_category=<?=$report_category?>&report_mb_id=<?=$report_mb_id?>&stx2=<?=$stx2?>'">
                        <span class="ov_txt <?=$report_type=="wr_42" ? "select_btn" : "" ?>">FBA창고</span>
                        <span class="ov_num"><?php echo number_format($cnt['fba']) ?>개</span>
                    </span>
                    <span class="btn_ov01" onclick="location.href='<?=G5_URL?>/report/report_list.php?report_type=wr_43&report_category=<?=$report_category?>&report_mb_id=<?=$report_mb_id?>&stx2=<?=$stx2?>'">
                        <span class="ov_txt <?=$report_type=="wr_43" ? "select_btn" : "" ?>">W-FBA창고</span>
                        <span class="ov_num"><?php echo number_format($cnt['wfba']) ?>개</span>
                    </span>
                    <span class="btn_ov01" onclick="location.href='<?=G5_URL?>/report/report_list.php?report_type=wr_44&report_category=<?=$report_category?>&report_mb_id=<?=$report_mb_id?>&stx2=<?=$stx2?>'">
                        <span class="ov_txt <?=$report_type=="wr_44" ? "select_btn" : "" ?>">U-FBA창고</span>
                        <span class="ov_num"><?php echo number_format($cnt['ufba']) ?>개</span>
                    </span>
                    <span class="btn_ov01" onclick="location.href='<?=G5_URL?>/report/report_list.php?report_type=wr_37&report_category=<?=$report_category?>&report_mb_id=<?=$report_mb_id?>&stx2=<?=$stx2?>'">
			...
			<ul class="<?php echo isset($view) ? 'view_is_list btn_top' : 'btn_top2';?>">
			    ...
			    <li>
                    <select id="sorting_box" class="frm_input" style="height:37px">
                        ...
                        <option value="up" <?=($_GET['sst']=="stock" && $sod =="desc") ? "selected" : ""?> >재고많은순</option>
                        <option value="down" <?=($_GET['sst']=="stock" && $sod =="asc") ? "selected" : ""?> >재고적은순</option>
                    </select>
				</li>
                ...
                <li><button type="button" class="btn_b01 mb_write_btn" data="<?=$member['mb_id']?>" style="margin:10px;" >담당자별 재고조사</button></li>
                <li><button type="button" class="btn_b01 rack_write_btn" style="margin:10px;" >랙별 재고조사</button></li>
			</ul>
		</div>
	    ...
	    <div id="bo_li_01" style="clear:both;overflow-x:scroll;overflow-y:hidden">
	    	...
	    	<ul class="list_head" style="width:100%;min-width:1576px;position:sticky;top:0;background:#fff;z-index:2;" >
                ...
                <li style="width:120px">실사재고조사</li>
	        </ul>
			<?php }?>
	        <div id="bo_li_01" class="list_03" >
		        <ul style="width:100%;">
		            ...
		            <li class="modify" data="<?php echo $row['seq']?>">
		                ...
		                <div class="cnt_left" style="width:100px;text-align:right"><?=number_format($row['wr_32'])?>개</div>
						<div class="cnt_left" style="width:100px;text-align:right"><?=number_format($row['wr_36'])?>개</div>
						<div class="cnt_left" style="width:100px;text-align:right"><?=number_format($row['wr_42'])?>개</div>
						<div class="cnt_left" style="width:100px;text-align:right"><?=number_format($row['wr_43'])?>개</div>
						<div class="cnt_left" style="width:100px;text-align:right"><?=number_format($row['wr_44'])?>개</div>
                        <div class="cnt_left" style="width:100px;text-align:right"><?=number_format($row['wr_37'])?>개</div>
						<div class="cnt_left" style="width:120px;text-align:right"><?=number_format($hab_cnt)?>개</div>
				    </li>
				    <?php 
					$cur_no = $cur_no - 1;
					} ?>
    ...
    <?php echo get_paging(G5_IS_MOBILE ? $config['cf_mobile_pages'] : $config['cf_write_pages'], $page, $total_page, $_SERVER['SCRIPT_NAME'] . '?' . $qstr."&amp;report_type={$report_type}&report_category={$report_category}&report_mb_id={$report_mb_id}" . '&amp;page='); ?>
</div>
...
<div class="modal_frm">
    <div class="modal_body">
        ...
        <button type="button" onclick="modal_action()" class="modal_cls_btn" title="닫기"><i class="fa fa-times" aria-hidden="true"></i><span class="sound_only">닫기</span></button>
    </div>
...
$(function() {
    ...
    $('#mb_id_box').bind('change', function(){
        ...
        location.href = '?sst=<?=$sst?>&sod=<?=$sod?>&stx2=<?php echo $stx2?>&report_type=<?=$report_type?>&report_category=<?=$report_category?>&report_mb_id='+val;
		
	});

    $('#category_box').bind('change', function(){
		let val = $(this).val();
        location.href = `?sst=<?=$sst?>&sod=<?=$sod?>&stx2=<?=$stx2?>&report_type=<?=$report_type?>&report_category=${val}&report_mb_id=<?=$report_mb_id?>`;
	});
	...
	    var _top = Math.ceil(( window.screen.height - _height )/2); 
		...
		window.open("./report_mb.php?mb_id="+id, "pop_rack"+id, "left="+_left+", top="+_top+", width="+_width+", height="+_height+", scrollbars=1");
