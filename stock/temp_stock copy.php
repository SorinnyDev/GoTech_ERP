	    ...
	    <div class="tbl_wrap tbl_head02" style="overflow-x:scroll;height:700px">
			<table>
				...
				<tbody>
					...
					LEFT JOIN g5_write_product c ON(c.wr_1 = b.wr_code OR b.wr_code = c.wr_27  OR b.wr_code = c.wr_28 OR b.wr_code = c.wr_29 OR b.wr_code = c.wr_30 OR b.wr_code = c.wr_31)
					where c.wr_37 > 0 AND a.wr_datetime > '2024-05-01 00:00:00' AND a.sales2_id IS NOT NULL {$sql_search} order by a.tw_seq desc";
					...
					?>
		            <tr class="modify" data="<?php echo $row['seq']?>">
		                ...
		                <td class="cnt_left" style="width:100px"><?php echo $row['wr_domain'] ?> 
		                ...
		                <td class="cnt_left" style="width:80px;text-align:right"><?php echo $row['wr_37']?></td>
		                <td class="cnt_left" style="width:80px;text-align:right"><?php echo $order_ea_txt.$order_ea_date?>
						...
						<td class="cnt_left" style="width:100px;text-align:right">
                            <input type="text" name="wr_37[<?php echo $i?>]" class="wr_37 frm_input " style="width:100%;text-align:right" value="<?php echo $order_ea ?>">
                        </td>
						<td class="cnt_left" style="width:100px;text-align:center">
                            <select name="wr_rack[<?php echo $i?>]" class="wr_rack frm_input search_sel">
								<?php if(!$row['wr_etc_use']) echo '<option value="">선택</option>'; ?>
                                <?php echo get_rack_option(1000, $sel_wr_rack)?>
                            </select>
						</td>
						<td class="cnt_left" style="width:100px;text-align:center">
							<?php if($row['wr_rack']) {?>
...
$(function() {
	...
	$('.warehouse').bind('change', function() {
		...
		if($(this).val() == "1000") {
			$(this).closest('tr').find('.wr_rack').html('<option value="">선택</option><?php echo get_rack_option(1000)?>');
		} else if($(this).val() == "3000") {
			$(this).closest('tr').find('.wr_rack').html('<option value="">선택</option><?php echo get_rack_option(3000)?>');
		}
