...
<div class="new_win">
                    ...
                    <tbody>
                        <tr>
                            <td>
                                ...
                                <select name="ms_rack" class="ms_rack required" required>
                                    ...
                                    ?>
                                    <option value="<?php echo $rack['seq']?>" data="<?php echo $stock['total']?>"><?php echo $rack['gc_name']?> (재고:<?php echo $stock['total']?>)</option>
                                    <?php 
                                    $chk++;
                                    }
                        ...
                        <tr>
                            <td class="down_arrow">
                                ...
                                <select name="ms_rack2" class="ms_rack2 required" required>
	...
	<div id="excelfile_upload" class="result_list" style="padding:12px;width:47%;float:left">
		<div class="tbl_head02 tbl_wrap" style="">
            <table>
                <tr>
                    <th style="width:100px">임시창고</th>
                    <td style="text-align:right"><?php echo number_format($it['wr_37'])?></td>
                    <th style="width:100px">전체재고</th>
                    <td style="text-align:right"><?php echo number_format($it['wr_32']+$it['wr_36']+$it['wr_37'])?></td>
                </tr>
            </table>
		</div>
		<div style="clear:both"></div>
		<div class="tbl_head01 tbl_wrap" style="width:49%; margin-right:1%;float:left">
            <table>
                <thead style="position:sticky;top:0;">
                    <tr>
                        <th style="width:100%" colspan="2"><strong>한국창고</strong> (<?php echo number_format($it['wr_32'])?>개)</th>
                    </tr>
                    <tr>
                        <th style="width:100%" colspan="2">
                            <input type="text" id="rack_kor_ser" class="frm_input rack_search" placeholder="랙 이름 검색" style="width:100%">
                            <script>
                            $(document).ready(function() {
                                $("#rack_kor_ser").keyup(function() {
                                    var k = $(this).val();
                                    
                                    $("#kor_rack_list > tr").hide();
                                    var temp = $("#kor_rack_list > tr > td:icontains('" + k + "')");
                ...
                <tbody id="kor_rack_list">
                    <?php
                    $kor_stock = 0;
                    $sql_common = " from g5_rack ";
                    $sql_search = " where gc_warehouse = 1000 and gc_use = 1 order by SUBSTRING(gc_name, 1, 1) asc";
                    $sql = " select * {$sql_common} {$sql_search}  ";
                    $result = sql_query($sql);
                    for($i=0; $row=sql_fetch_array($result); $i++) {
                    ...
                    <tr class="<?php echo $bg; ?>">
                        ...
                        <td>
                            <form name="frmIndi<?php echo $row['seq']?>" action="./rack_move_update.php" method="post">
                                ...
                                <input type="hidden" name="wr_id" value="<?php echo $wr_id?>">
                                <input type="hidden" name="warehouse" value="1000">
                                <input type="hidden" name="seq" value="<?php echo $row['seq']?>">
                                <input type="hidden" name="ori_qty" value="<?php echo (int)$stock['total']?>">
                                <input type="number" name="qty" class="frm_input qty" value="<?php echo (int)$stock['total']?>" style="text-align:right;width:30%" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');">
		...
		<div class="tbl_head01 tbl_wrap" style="width:49%;float:left">
            <table>
                <thead style="position:sticky;top:0;">
                    <tr>
                        <th style="width:100%" colspan="2"><strong>미국창고</strong> (<?php echo number_format($it['wr_36'])?>개)</th>
                    </tr>
                    <tr>
                        <th style="width:100%" colspan="2">
                            <input type="text" id="rack_eng_ser" class="frm_input rack_search" placeholder="랙 이름 검색" style="width:100%">
                            <script>
                            $(document).ready(function() {
                                
                                
                                $("#rack_eng_ser").keyup(function() {
                                    var k = $(this).val();
                                    
                                    $("#eng_rack_list > tr").hide();
                                    var temp = $("#eng_rack_list > tr > td:icontains('" + k + "')");
                    ...
                    <tr>
                        ...
                        <td>
                            <form name="frmIndi<?php echo $row['seq']?>" action="./rack_move_update.php" method="post">
                                <input type="hidden" name="mode" value="indi">
                                <input type="hidden" name="wr_id" value="<?php echo $wr_id?>">
                                <input type="hidden" name="warehouse" value="3000">
                                <input type="hidden" name="seq" value="<?php echo $row['seq']?>">
                                <input type="hidden" name="ori_qty" value="<?php echo (int)$stock['total']?>">
                                <input type="number" name="qty" class="frm_input qty" value="<?php echo (int)$stock['total']?>" style="text-align:right;width:30%" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');">
                                <button class="btn01">수정</button>
	...
	<div id="excelfile_upload" class="result_list" style="padding:12px;width:47%;float:left">
		...
		<div class="tbl_head01 tbl_wrap" style="width:49%; margin-right:1%;float:left">
            <table>
                <thead style="position:sticky;top:0;">
                    <tr>
                        <th style="width:100%" colspan="2"><strong>한국 반품창고</strong> (<?php echo number_format($it['wr_40'])?>개)</th>
                    </tr>
                    <tr>
                        <th style="width:100%" colspan="2">
                            <input type="text" id="rack_kor2_ser" class="frm_input rack_search" placeholder="랙 이름 검색" style="width:100%">
                            <script>
                            $(document).ready(function() {
                                $("#rack_kor2_ser").keyup(function() {
                                    var k = $(this).val();
                                    
                                    $("#kor2_rack_list > tr").hide();
                                    var temp = $("#kor2_rack_list > tr > td:icontains('" + k + "')");
                ...
                <tbody id="kor2_rack_list">
                    <?php
                    $kor_stock = 0;
                    $sql_common = " from g5_rack ";
                    $sql_search = " where gc_warehouse = 7000 and gc_use = 1 order by gc_name asc";
                    $sql = " select * {$sql_common} {$sql_search}  ";
                    $result = sql_query($sql);
                    for($i=0; $row=sql_fetch_array($result); $i++) {
                    ...
                    <tr class="<?php echo $bg; ?>">
                        ...
                        <td>
                            <form name="frmIndi<?php echo $row['seq']?>" action="./rack_move_update.php" method="post">
                                <input type="hidden" name="mode" value="indi">
                                <input type="hidden" name="wr_id" value="<?php echo $wr_id?>">
                                <input type="hidden" name="warehouse" value="7000">
                                <input type="hidden" name="seq" value="<?php echo $row['seq']?>">
                                <input type="hidden" name="ori_qty" value="<?php echo (int)$stock['total']?>">
                                <input type="number" name="qty" class="frm_input qty" value="<?php echo (int)$stock['total']?>" style="text-align:right;width:30%" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');">
                                <button class="btn01">수정</button>
		...
		<div class="tbl_head01 tbl_wrap" style="width:49%;float:left">
            <table>
                <thead style="position:sticky;top:0;">
                <tr>
                    <th style="width:100%" colspan="2"><strong>미국 반품창고</strong> (<?php echo number_format($it['wr_41'])?>개)</th>
                </tr>
                <tr>
                    <th style="width:100%" colspan="2">
                    <input type="text" id="rack_eng2_ser" class="frm_input rack_search" placeholder="랙 이름 검색" style="width:100%">
                    <script>
                    $(document).ready(function() {
                        $("#rack_eng2_ser").keyup(function() {
                            var k = $(this).val();
                            
                            $("#eng2_rack_list > tr").hide();
                            var temp = $("#eng2_rack_list > tr > td:icontains('" + k + "')");
                ...
                <tbody id="eng2_rack_list">
                    <?php
                    $usa_stock = 0;
                    $sql_common = " from g5_rack ";
                    $sql_search = " where gc_warehouse = 8000 and gc_use = 1 order by gc_name asc";
                    $sql = " select * {$sql_common} {$sql_search}  ";
                    $result = sql_query($sql);
                    for($i=0; $row=sql_fetch_array($result); $i++) {
                        $stock = sql_fetch("select *, SUM(wr_stock) as total from g5_rack_stock where wr_warehouse = '{$row['gc_warehouse']}' and wr_rack = '{$row['seq']}' and wr_product_id = '{$wr_id}' ");
                    ...
                    <tr>
                        ...
                        <input type="hidden" name="mode" value="indi">
                        <input type="hidden" name="wr_id" value="<?php echo $wr_id?>">
                        <input type="hidden" name="warehouse" value="8000">
                        <input type="hidden" name="seq" value="<?php echo $row['seq']?>">
                        <input type="hidden" name="ori_qty" value="<?php echo (int)$stock['total']?>">
                        <input type="number" name="qty" class="frm_input qty" value="<?php echo (int)$stock['total']?>" style="text-align:right;width:30%" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');">
                        <button class="btn01">수정</button>
