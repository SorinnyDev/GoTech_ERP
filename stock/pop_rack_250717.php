...
// $warehouseConfig 배열을 순회하며 품질에 따라 분류
foreach ($warehouseConfig as $warehouse_id => $product_details) {
    switch ($product_details['quality']) {
        case 'good':
            $good_product[] = (int)$warehouse_id; // 숫자로 변환하여 추가
            break;
        case 'refur':
            $refur_product[] = (int)$warehouse_id; // 숫자로 변환하여 추가
            break;
        case 'dispose':
            $dispose_product[] = (int)$warehouse_id; // 숫자로 변환하여 추가
            break;
    }
}
...
?>
              ...
              <td>
                ...
                <select name="ms_rack" class="ms_rack required" required>
                  ...
                  for ($a = 0; $rack = sql_fetch_array($result); $a++) {
                    $rack_sql = "select *, SUM(wr_stock) as total from g5_rack_stock where 
                                wr_warehouse = '{$rack['gc_warehouse']}' and wr_rack = '{$rack['seq']}' and wr_product_id = '{$wr_id}' ";
                    $stock = sql_fetch($rack_sql);

                        if ($stock['total'] <= 0) continue;

                    // echo "select *, SUM(wr_stock) as total from g5_rack_stock where wr_warehouse = '{$rack['gc_warehouse']}' and wr_rack = '{$rack['seq']}' and wr_product_id = '{$wr_id}' ";
  ...
  <script>
    ...
    async function fetch_pop_rack_container() {
      ...
      // const group2Keys = [7000, 8000, 9000, 9100, 9200];
      ...
      // const group3Keys = [11000, 12000, 13000, 13100, 13200];
    ...
    $(document).on('keyup', '.rack_search', function () {
      ...
      $rows.hide().filter(function () {
        return $(this).find('td').text().indexOf(k) > -1;
      }).show();
    });
