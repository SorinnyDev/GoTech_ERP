...
?>
    ...
    <div class="tbl_head01 tbl_wrap" style="width:20%; float:left;height:390px;overflow-y:scroll;">
      <table>
      ...
      for ($i = 0; $row = sql_fetch_array($result); $i++) {
        if ($row['total_stock'] === '0' && $zero_first < $i) break;
        ?>
          <tr>
            <td><?= $row['gc_name'] ?></td>
            <td class="flex-center-content" style="padding: 0">
        ...
        <?php if ($row['total_stock'] !== '0' && $zero_first === $i) ++$zero_first;
      }

      if (($row_count - $zero_first - 1) > 0) { ?>
          <tr>
            <td colspan="2" style="height: 46px;">나머지 재고 0인 랙 <?= $row_count - $zero_first - 1 ?>개</td>
          </tr>
        <?php }
      if (!$only_tr) { ?>
        </tbody>
