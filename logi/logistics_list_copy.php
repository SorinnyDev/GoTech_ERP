              ...
              <ul style="width:100%;min-width:max-content;height: 580px;overflow:auto;">
                ...
                foreach ($list as $k => $value) {
                  ...
                  <?php foreach ($value as $item) {
                    ...
                    if (!$item['wr_delivery_oil']) {
                      $country_dcode = sql_fetch("SELECT wr_code AS code FROM g5_country WHERE code_2 = '{$item['wr_deli_country']}'");
                      ...
                      $sql = "SELECT {$country} AS price, cust_code, weight_code,B.code_percent FROM g5_shipping_price A
                          ...
                          WHERE weight_code >= {$item['wr_weight2']} and {$country} != 0 and cust_code = '{$item['wr_delivery']}'  group by cust_code order by price asc";

                      $result = sql_fetch($sql);

                      $wr_delivery_oil_percent = $result['code_percent'];
                      $wr_delivery_oil = $result['price'] * $wr_delivery_oil_percent / 100;

                      $item['wr_delivery_oil'] = $wr_delivery_oil;
                    }
